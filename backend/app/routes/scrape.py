...
def _run_discovery(payload: ScrapeRequest):
    try:
        ...
        with SessionLocal() as db:
            ...
            threshold = float(threshold_row.value) if threshold_row else get_settings().score_threshold
            raw_jobs = crawl(
                payload.query, payload.days, target=payload.max_jobs,
            ...
            existing = {
                job.external_id: job
                for job in db.scalars(select(Job).where(Job.external_id.in_(extracted_jobs))).all()
            } if extracted_jobs else {}
            new = updated = above = 0
            ...
            db.commit()
            result = {"fetched": len(raw_jobs), "new": new, "updated": updated,
...
def discover(payload: ScrapeRequest, background: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    if not resume or not resume.value.strip():
        raise HTTPException(400, "Save your resume before discovering jobs so Hirelight can score matches.")
    with _lock:
        if _status["running"]:
