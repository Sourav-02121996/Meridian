...
SKILLS = {
    "python", "javascript", "typescript", "java", "kotlin", "swift", "golang", "go", "rust",
    "c++", "c#", "react", "angular", "vue", "next.js", "node.js", "django", "flask", "fastapi",
    "spring", "sql", "postgresql", "mysql", "mongodb", "redis", "graphql", "rest", "aws", "azure",
    "gcp", "docker", "kubernetes", "terraform", "linux", "git", "ci/cd", "pytorch", "tensorflow",
    "machine learning", "spark", "kafka", "snowflake", "databricks", "html", "css", "tailwind",
}
HEADER = re.compile(r"^(requirements?|qualifications?|what you(?:'|’)ll need|must have|skills|responsibilities)\b", re.I)
...
def score_job(description: str, resume: str) -> dict:
    if not description.strip() or not resume.strip():
        return {"score": 0.0, "requirement_coverage": 0.0, "skill_coverage": 0.0,
                "global_similarity": 0.0, "matched_skills": [], "missing_skills": _skills(description),
    ...
    if requirements and resume_lines:
        ...
        weak = [requirement for requirement, similarity in zip(requirements, best) if similarity < 0.45]
    ...
    score = round((.55 * req_cov + .35 * skill_cov + .10 * global_sim) * 100, 1)
    return {"score": score, "requirement_coverage": req_cov, "skill_coverage": skill_cov,
            "global_similarity": global_sim, "matched_skills": matched,
            "missing_skills": missing, "weak_requirements": weak}
